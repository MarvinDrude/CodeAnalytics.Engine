...
public static class MethodSymbolConnector
{
   public static void Connect(BakeContext context)
   {
      var targetName = context.FileNames[FileIds.MethodSymbol];
      ...
      var targetCount = (int)(targetHandle.Length / Unsafe.SizeOf<MethodSymbolSpec>());
      var targetSymbols = targetBuffer.GetSpan<MethodSymbolSpec>(0, targetCount);
      ...
      while (targetIndex < targetCount)
      {
         ...
         var parameters = new StorageView<ParameterSymbolSpec>(-1, 0);
         var typeParameters = new StorageView<TypeParameterSymbolSpec>(-1, 0);
         
         targetSymbol.Parameters = parameters;
         ...
         while (edgeIndex < edgeCount && edges[edgeIndex].SourceSymbolId == currentSourceId)
         {
            ...
            switch (currentType)
            {
               case SymbolEdgeType.Parameter:
                  parameters = new StorageView<ParameterSymbolSpec>(typeStartIndex, typeCount);
                  break;
