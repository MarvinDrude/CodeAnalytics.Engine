...
public static class FolderConnector
{
   public static void Connect(BakeContext context)
   {
      var targetName = context.FileNames[FileIds.Folder];
      ...
      var targetCount = (int)(targetHandle.Length / Unsafe.SizeOf<FolderSpec>());
      var targetSymbols = targetBuffer.GetSpan<FolderSpec>(0, targetCount);
      ...
      while (targetIndex < targetCount)
      {
         ...
         var subFolders = new StorageView<FolderSpec>(-1, 0);
         var files = new StorageView<FileSpec>(-1, 0);
         
         targetSymbol.SubFolders = subFolders;
         ...
         while (edgeIndex < edgeCount && edges[edgeIndex].SourceSymbolId == currentSourceId)
         {
            ...
            switch (currentType)
            {
               case SymbolEdgeType.FolderToFile:
                  files = new StorageView<FileSpec>(typeStartIndex, typeCount);
                  break;
               case SymbolEdgeType.FolderToFolder:
                  subFolders = new StorageView<FolderSpec>(typeStartIndex, typeCount);
                  break;
            }
         }
